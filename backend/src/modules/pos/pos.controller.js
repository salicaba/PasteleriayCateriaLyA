    ...
    <body class="text-gray-800 antialiased flex flex-col items-center justify-center min-h-screen p-4 sm:p-6 select-none">
      ...
      <div id="ticket-download-area" class="w-full max-w-md flex flex-col items-center justify-center p-2 bg-transparent">
        <div id="ticket-card" class="w-full bg-white rounded-[2.5rem] shadow-xl shadow-slate-100 border border-slate-100 p-6 sm:p-8 relative transition-all duration-300">
          ...
          <div class="mb-6 p-4 bg-amber-50 rounded-2xl border border-amber-100 no-print">
            <label class="block text-xs font-black text-amber-800 uppercase mb-2">Ver cuenta específica:</label>
            <select onchange="cambiarCuenta(this.value)" class="w-full p-3 bg-white text-sm font-bold text-amber-900 rounded-xl border border-amber-200 focus:ring-2 focus:ring-amber-500 outline-none transition-all cursor-pointer">
              <option value="General" ${!cuenta || cuenta === 'General' ? 'selected' : ''}>● Ver todas las cuentas</option>
              ${cuentasUnicas.filter(c => c !== 'General').map(c => `
                <option value="${c}" ${cuenta === c ? 'selected' : ''}>● Cuenta: ${c}</option>
              `).join('')}
            </select>
          </div>
          ` : ''}
          ...
          <div class="space-y-2 text-sm font-medium text-slate-600 mb-6">
            ...
            <div class="flex justify-between items-center bg-amber-100 px-2 py-1 rounded-md">
              <span class="text-amber-800">Filtrando por:</span>
              <span class="text-amber-900 font-black uppercase">${cuenta}</span>
            </div>` : ''}
          </div>
      ...
      <script>
        function cambiarCuenta(nombre) {
            window.location.href = window.location.pathname + '?cuenta=' + encodeURIComponent(nombre);
        }
