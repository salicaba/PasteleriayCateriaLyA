  ...
  try {
    ...
    const order = await Order.findByPk(orderId, {
      include: [
        ...
        { model: OrderItem, as: 'items', include: [{ model: Product, as: 'product', attributes: ['name', 'basePrice'] }] }
      ]
    });

    if (!order) return res.status(404).send('<h1>Ticket no encontrado</h1>');

    // -- Lógica inicial --
    let cashierName = 'Sistema';
    // ... (tu lógica de obtener cashierName) ...

    let itemsFiltrados = order.items || [];
    if (cuenta && cuenta !== 'General') itemsFiltrados = itemsFiltrados.filter(i => i.cuenta === cuenta);
    const totalAmount = itemsFiltrados.reduce((sum, item) => sum + Number(item.subtotal), 0);
    const cuentasUnicas = Array.from(new Set(itemsFiltrados.map(i => i.cuenta || 'General')));
    const dateStr = new Date(order.createdAt).toLocaleDateString('es-MX', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' });
    let isLlevar = order.orderType === 'LLEVAR';
    let identificadorMesa = isLlevar ? `Pedido #${order.ticketId?.split(' - ')[0] || 'Llevar'}` : `Mesa #${order.table?.number || 'Salón'}`;

    // --- CONSTRUCCIÓN DEL HTML DE DETALLES (SEPARADO PARA EVITAR ERRORES) ---
    let detallesHtml = "";
    
    cuentasUnicas.forEach(accName => {
        if (cuenta && cuenta !== 'General' && accName !== cuenta) return;
        const accountItemsRaw = itemsFiltrados.filter(i => (i.cuenta || 'General') === accName);
        if (accountItemsRaw.length === 0) return;

        const grouped = [];
        accountItemsRaw.forEach(item => {
            const key = `${item.productId}-${item.isTakeaway}-${item.notes}`;
            const existing = grouped.find(g => g.key === key);
            if (existing) { existing.quantity += item.quantity; existing.subtotal += Number(item.subtotal); }
            else grouped.push({ ...item.toJSON(), key, quantity: item.quantity, subtotal: Number(item.subtotal) });
        });

        if (cuentasUnicas.length > 1 && !cuenta) {
            detallesHtml += `<div class="text-[10px] font-black text-slate-400 bg-slate-50 px-3 py-1 rounded-lg uppercase tracking-wide mb-2">● Cuenta: ${accName}</div>`;
        }
        ...
        grouped.forEach(item => {
            ...
            <div class="flex items-start gap-3 text-sm px-1 mb-2">
                ...
                <div class="flex-1 min-w-0">
                    <p class="font-bold text-slate-900">${item.isTakeaway ? '<span class="text-orange-500 text-[10px] bg-orange-50 px-1 rounded mr-1">🛍️</span>' : ''}${item.product.name}</p>
                </div>
                <span class="font-bold text-slate-900">$${item.subtotal.toFixed(2)}</span>
            </div>`;
        });
    });
    ...
    <head>
      ...
      <title>Tu Ticket - 𝓛𝔂𝓪</title>
      <script src="https://cdn.tailwindcss.com"></script>
      <script src="https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js"></script>
      <script src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js"></script>
    </head>
    <body class="bg-slate-50 p-4 flex flex-col items-center min-h-screen">
      <div id="ticket-card" class="w-full max-w-sm bg-white p-6 rounded-3xl shadow-xl border border-slate-100">
        <div class="text-center mb-6">
            <h1 class="text-5xl font-black text-amber-600 mb-2 italic" style="font-family: serif;">𝓛𝔂𝓪</h1>
            <p class="text-xs uppercase font-bold text-slate-400">Pastelería & Cafetería</p>
        </div>

        ${cuentasUnicas.length > 1 ? `
          <div class="mb-6 p-4 bg-amber-50 rounded-2xl border border-amber-100 no-print">
            <select onchange="window.location.href='?cuenta='+this.value" class="w-full p-2 text-sm font-bold bg-white rounded-lg border border-amber-200">
              <option value="General">Ver todas las cuentas</option>
              ${cuentasUnicas.filter(c => c !== 'General').map(c => `<option value="${c}" ${cuenta === c ? 'selected' : ''}>Cuenta: ${c}</option>`).join('')}
            </select>
          </div>
        ` : ''}

        <div class="space-y-4 mb-6">
           <p class="text-xs font-bold text-slate-400 uppercase">Detalle de consumo</p>
           ${detallesHtml}
        </div>

        <div class="border-t-2 border-dashed pt-4 text-center">
          <p class="text-2xl font-black text-slate-900">Total: $${totalAmount.toFixed(2)}</p>
        </div>
      </div>

      <div class="flex gap-3 mt-6 no-print w-full max-w-sm px-4">
        <button onclick="descargarPDF()" class="flex-1 bg-black text-white py-4 rounded-xl font-black text-xs uppercase">📥 PDF</button>
        <button onclick="descargarImagen()" class="flex-1 bg-amber-600 text-white py-4 rounded-xl font-black text-xs uppercase">📸 Imagen</button>
      </div>

      <script>
        function descargarPDF() {
            const el = document.getElementById('ticket-card');
            const h = (el.scrollHeight * 0.264583) + 15;
            html2pdf().set({ margin:0, filename: 'Ticket.pdf', jsPDF: { unit: 'mm', format: [80, h] } }).from(el).save();
        }
        function descargarImagen() {
            html2canvas(document.getElementById('ticket-card'), {scale: 3, backgroundColor: '#ffffff'}).then(c => {
                const link = document.createElement('a'); link.download = 'Ticket.png'; link.href = c.toDataURL(); link.click();
            });
        }
      </script>
    </body>
    ...
    res.status(500).send('<h3>Error al renderizar el ticket</h3>');
  }
};