...
import { CalendarClock, Plus, Cake, AlertCircle, CheckCircle2, DollarSign, X, FileText } from 'lucide-react';
...
export default function PasteleriaDashboard() {
  ...
  return (
    <div className="p-6 h-full flex flex-col gap-6 relative">
      ...
      {/* HEADER DEL MÓDULO */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/10 dark:bg-black/20 backdrop-blur-md border border-white/20 dark:border-white/10 p-4 rounded-2xl shadow-xl shrink-0">
        <div>
        ...
        <motion.button 
          whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setIsModalOpen(true)}
          className="w-full sm:w-auto bg-gradient-to-r from-emerald-500 to-teal-500 text-white px-6 py-2.5 rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/30 font-bold"
        >
      ...
      {/* MODALES */}
      <NuevoPedidoModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSave={agregarNuevoPedido} />
