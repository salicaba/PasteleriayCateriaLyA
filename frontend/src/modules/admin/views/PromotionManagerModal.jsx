...
export default function PromotionManagerModal({ isOpen, onClose, product, editData, onPromotionSaved }) {
  ...
  // Reseteamos el formulario cada vez que se abre para crear una NUEVA promoción o EDITAR
  useEffect(() => {
    if (isOpen) {
      if (editData) {
        setFormData({
            ...
            <div className="p-6 md:p-8 border-t border-gray-200 dark:border-gray-800 lya:border-lya-border/40 shrink-0 bg-white dark:bg-gray-900 lya:bg-lya-surface">
              ...
              >
                {isProcessing ? (
                  ...
                  <>
                    <Save size={24} strokeWidth={2.5} /> {editData ? 'Guardar Cambios' : 'Guardar Promoción'}
                  </>
                )}
              </motion.button>
            </div>
