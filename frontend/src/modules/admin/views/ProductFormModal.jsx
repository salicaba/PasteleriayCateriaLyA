...
export const ProductFormModal = ({ initialData, onClose, onSave, categories = [], globalOptions = [] }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 lya:bg-black/50 backdrop-blur-md overflow-y-auto">
      <div className="bg-white dark:bg-gray-900 lya:bg-lya-surface w-full max-w-4xl rounded-3xl overflow-hidden shadow-2xl my-auto transition-colors">
        
        {imageSrc ? (
          <div className="relative h-[500px] w-full bg-gray-950 lya:bg-black/90">
            <Cropper
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-black/50 p-4 rounded-2xl backdrop-blur-md z-10">
              ...
              <input type="range" value={zoom} min={1} max={3} step={0.1} onChange={(e) => setZoom(e.target.value)} className="w-32 accent-orange-500 lya:accent-lya-primary" />
              <button onClick={handleApplyCrop} className="bg-orange-500 hover:bg-orange-600 lya:bg-lya-primary lya:hover:bg-lya-primary/90 transition-colors text-white lya:text-lya-surface px-6 py-2 rounded-xl font-bold">Aplicar Recorte</button>
            </div>
          </div>
        ) : (
            ...
            <div className="p-8 overflow-y-auto custom-scrollbar space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="md:col-span-1">
                  <div className="flex flex-col items-center justify-center border-2 border-dashed border-gray-200 dark:border-gray-700 lya:border-lya-border/40 rounded-3xl p-6 hover:border-orange-500 lya:hover:border-lya-primary transition-colors bg-gray-50/50 dark:bg-gray-800/30 lya:bg-lya-bg/50 h-full">
                    {formData.imageUrl ? (
                      <img src={formData.imageUrl} className="w-40 h-40 object-cover rounded-2xl mb-4 shadow-md" alt="Product" />
                    ) : (
                      <div className="w-40 h-40 bg-gray-200 dark:bg-gray-800 lya:bg-lya-bg rounded-2xl mb-4 flex items-center justify-center">
              ...
              <div className="border-t border-gray-200 dark:border-gray-800 lya:border-lya-border/30 pt-6">
                <h3 className="font-black text-xl text-gray-800 dark:text-white lya:text-lya-text mb-1 flex items-center gap-2">
                  <Settings2 size={24} className="text-blue-500 lya:text-lya-secondary" />
                  Personalización del Producto
                </h3>
                <p className="text-gray-500 lya:text-lya-text/60 text-sm mb-6">Marca las opciones disponibles y asigna el precio extra para este producto.</p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  {['tamanos', 'leches', 'extras'].map((tipo) => {
                    ...
                    return (
                      <div key={tipo} className="bg-gray-50 dark:bg-gray-800/50 lya:bg-lya-bg/50 p-5 rounded-3xl border border-gray-100 dark:border-gray-800 lya:border-lya-border/30 transition-colors">
                        <h4 className="font-bold text-gray-700 dark:text-gray-200 lya:text-lya-text capitalize mb-4">{tipo}</h4>
                        
                        {optsDisponibles.length === 0 ? (
                           <p className="text-xs text-gray-400 lya:text-lya-text/50 italic">No hay {tipo} en el catálogo global.</p>
                        ) : (
                                 ...
                                 >
                                   ...
                                   >
                                     <div className="mt-0.5 shrink-0">
                                       {isSelected ? (
                                         <CheckCircle2 size={18} className="text-orange-500 lya:text-lya-primary" />
                                       ) : (
                                         <div className="w-[18px] h-[18px] rounded-full border-2 border-gray-300 dark:border-gray-600 lya:border-lya-border/50" />
                                       )}
                                     </div>
                                   ...
                                   {isSelected ? (
                                     <div className="mt-3 pt-3 border-t border-orange-200/50 dark:border-orange-500/20 lya:border-lya-primary/30 flex items-center justify-between animate-in fade-in slide-in-from-top-1">
                                       <span className="text-xs font-bold text-orange-600 dark:text-orange-400 lya:text-lya-primary opacity-80">Costo extra:</span>
                                     ...
                                     opt.precioAdicional > 0 && (
                                       <div className="mt-2 pl-7 text-[11px] opacity-50 font-bold text-gray-500 dark:text-gray-400 lya:text-lya-text/60">
                        ...
                        {(tipo === 'tamanos' || tipo === 'leches') && optsSeleccionadas.length > 0 && (
                          <div className="mt-4 p-3 bg-orange-50 dark:bg-orange-500/10 lya:bg-lya-primary/5 rounded-2xl border border-orange-100 dark:border-orange-500/20 lya:border-lya-primary/20">
                            <label className="text-[11px] font-black text-orange-600 dark:text-orange-400 lya:text-lya-primary uppercase flex items-center gap-1 mb-2">
