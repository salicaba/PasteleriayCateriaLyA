...
export const TicketCartGroup = ({
  ...
  // 🔥 CANDADOS DE ACCIONES DE CUENTA
  const isCobrarProcessing = actionLocks[`${cuentaName}-cobrar`];
  const isOcultarProcessing = actionLocks[`${cuentaName}-ocultar`];
  const isOcultarPagadaProcessing = actionLocks[`${cuentaName}-ocultar-pagada`]; // 🔥 TRAMPA 2 DESCUBIERTA
  const isPrintProcessing = actionLocks[`${cuentaName}-print`]; // 🔥 TRAMPA 1 DESCUBIERTA
  ...
  return (
    <motion.div 
      ...
      transition={{ duration: 0.2, ease: "easeOut" }} // 🔥 FIX: Transición directa sin resorte
      onDragOver={(e) => { 
        e.preventDefault(); 
        ...
        <div className="flex flex-col items-end gap-1 pointer-events-none">
          ...
          <div className="flex gap-1.5 flex-wrap justify-end pointer-events-auto">
            {displayItems.length === 0 && cuentaName !== 'General' && (
              <motion.button 
                  ...
                  className={clsx(
                    ...
                    isOcultarProcessing && "opacity-70 cursor-wait"
                  )}
              >
                {isOcultarProcessing ? <Loader2 size={10} className="animate-spin" /> : <Trash2 size={10}/>} Ocultar
              </motion.button>
            )}
            ...
            {!isCuentaPagada && !isCompletamentePagada && availableAccs.length > 1 && subtotalCuenta > 0 && !isVitrina && (
              ...
              >
                {isCobrarProcessing && <Loader2 size={10} className="animate-spin" />} Cobrar
              </motion.button>
            )}
            
            {/* 🔥 FIX: EL BOTÓN TRAMPA 1 AHORA ESTÁ BLOQUEADO */}
            ...
            {/* 🔥 FIX: EL BOTÓN TRAMPA 2 AHORA DEPENDE DE isOcultarPagadaProcessing */}
      ...
      <div className="px-2 pb-2 space-y-2">
        {sortedDisplayItems.map((item, index) => {
          ...
          <motion.div 
            key={currentItemKey} 
            layout="position"
            transition={{ duration: 0.2, ease: "easeOut" }} // 🔥 FIX: Listado suave
