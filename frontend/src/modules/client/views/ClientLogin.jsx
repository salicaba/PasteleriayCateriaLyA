...
// 🔥 Importamos el logo de 𝓛𝔂𝓪 para la pantalla de carga (Asegúrate de que la ruta sea correcta)
import logoLyA from '../../../assets/logo.jpeg'; 

export default function ClientLogin({ onLogin, isSubmitting, type, tableId }) {
  const [name, setName] = useState('');
  const [phone, setPhone] = useState(''); 
  const [showLoadingScreen, setShowLoadingScreen] = useState(false); // 🔥 Nuevo estado para la carga
  ...
  const handleSubmit = async (e) => {
    ...
    if (name.trim() && phone.trim()) {
      setShowLoadingScreen(true); // 1. Mostramos la pantalla de carga elegante
      
      // 2. Simulamos un pequeñísimo retraso visual (para que se vea la animación)
      await new Promise(resolve => setTimeout(resolve, 1500)); 
      
      // 3. Ejecutamos el login real
      onLogin({ 
        name: name.trim(), 
        phone: phone.trim() 
      });
  ...
  return (
    // 🔥 Cambiamos a h-[100dvh] para móviles, ocupando la pantalla entera siempre
      ...
      <AnimatePresence>
        {showLoadingScreen && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 z-[100] flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-950 lya:bg-lya-bg backdrop-blur-md"
          >
        ...
        {!showLoadingScreen && (
          <>
            ...
            <motion.div 
              ...
              // 🔥 Eliminamos el max-h-[90vh] y permitimos que la caja ocupe lo necesario
