      ...
      <div className={`p-4 flex flex-col ${headerColor} shadow-sm z-10 relative`}>
        <div className="flex justify-between items-start">
          <div className="flex flex-col">
            ...
            <span className="text-xs font-bold tracking-widest uppercase opacity-90 mt-2 bg-black/20 self-start px-2 py-0.5 rounded-md">
              Tanda {order.batch}
            </span>
          </div>
              ...
              >
                ...
                <div className="flex-1 text-sm">
                  ...
                  {prep.extras && prep.extras.length > 0 && (
                    <div className={`mt-1.5 flex items-start gap-1 p-1.5 rounded-md ${prep.isReady ? 'bg-transparent' : 'bg-orange-50 dark:bg-orange-900/20 border border-orange-100 dark:border-orange-800/50'}`}>
                      {!prep.isReady && <AlertTriangle size={14} className="text-orange-500 mt-0.5 shrink-0" />}
                      <span className="text-orange-600 dark:text-orange-400 font-bold text-xs leading-tight">
                        {prep.extras.join(', ')}
                      </span>
                    </div>
                  )}
